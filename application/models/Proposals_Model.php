...
class Proposals_Model extends CI_Model {
	...
	function proposal_add( $params ) {
		...
		$this->db->insert( 'logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '<a href="staff/staffmember/' . $loggedinuserid . '"> ' . $staffname . '</a> ' . lang( 'added' ) . ' <a href="proposals/proposal/' . $proposal . '">' . $appconfig['proposal_prefix'] . '' . $proposal.$appconfig['proposal_suffix'] . '</a>.' ),
	...
	function update_proposals( $id, $params ) {
		...
		$this->db->insert( 'logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '<a href="staff/staffmember/' . $loggedinuserid . '"> ' . $staffname . '</a> ' . lang( 'updated' ) . ' <a href="proposals/proposal/' . $id . '">' . $appconfig['proposal_prefix'] . '' . $id.$appconfig['proposal_suffix'] . '</a>.' ),
			'staff_id' => $loggedinuserid,
	...
	function delete_proposals( $id ) {
		...
		$this->db->insert( 'logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '<a href="staff/staffmember/' . $loggedinuserid . '"> ' . $staffname . '</a> ' . lang( 'deleted' ) . ' ' . $appconfig['proposal_prefix'] . '' . $id.$appconfig['proposal_suffix'] . '' ),
			'staff_id' => $loggedinuserid
		) );
	}
