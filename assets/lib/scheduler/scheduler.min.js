if (typeof CiuisScheduler === 'undefined') {
	var CiuisScheduler = {};
};
(function () {
	if (typeof CiuisScheduler.$ !== 'undefined') {
	...
	if (typeof CiuisScheduler.Global === "undefined") {
		CiuisScheduler.Global = {};
	};
	CiuisScheduler.$ = function (id) {
		return document.getElementById(id);
	};
	CiuisScheduler.isKhtml = (navigator && navigator.userAgent && navigator.userAgent.indexOf("KHTML") !== -1);
	CiuisScheduler.isIE = (navigator && navigator.userAgent && navigator.userAgent.indexOf("MSIE") !== -1);
	CiuisScheduler.mo2 = function ($a, ev) {
		ev = ev || window.event;
	...
	CiuisScheduler.mo3 = function ($a, ev, $e) {
		ev = ev || window.event;
		if (typeof (ev.pageX) !== 'undefined') {
			var $f = CiuisScheduler.abs($a, $e);
			var $b = {
				x: ev.pageX - $f.x,
				y: ev.pageY - $f.y
			};
		...
		return CiuisScheduler.mo2($a, ev);
	};
	CiuisScheduler.browser = {};
	CiuisScheduler.browser.ie9 = (function () {
		var $g = document.createElement("div");
		$g.innerHTML = "<!--[if IE 9]><i></i><![endif]-->";
		var $h = ($g.getElementsByTagName("i").length === 1);
		return $h;
	})();
	CiuisScheduler.browser.ielt9 = (function () {
		var $g = document.createElement("div");
		$g.innerHTML = "<!--[if lt IE 9]><i></i><![endif]-->";
		var $h = ($g.getElementsByTagName("i").length === 1);
		return $h;
	})();
	CiuisScheduler.abs = function (element, $i) {
		if (!element) {
			return null;
		};
		var r = {
		...
		if (element.getBoundingClientRect) {
			...
			r.x = b.left;
			r.y = b.top;
			var d = CiuisScheduler.doc();
			r.x -= d.clientLeft || 0;
			r.y -= d.clientTop || 0;
			var $j = CiuisScheduler.pageOffset();
			r.x += $j.x;
			r.y += $j.y;
			if ($i) {
				var $k = CiuisScheduler.absOffsetBased(element, false);
				var $i = CiuisScheduler.absOffsetBased(element, true);
				r.x += $i.x - $k.x;
				r.y += $i.y - $k.y;
				r.w = $i.w;
				r.h = $i.h;
			...
			return CiuisScheduler.absOffsetBased(element, $i);
		}
	};
	CiuisScheduler.absOffsetBased = function (element, $i) {
		var r = {
			x: element.offsetLeft,
			y: element.offsetTop,
			w: element.clientWidth,
	...
	CiuisScheduler.doc = function () {
		var de = document.documentElement;
		return (de && de.clientHeight) ? de : document.body;
	};
	CiuisScheduler.guid = function () {
		var S4 = function () {
			return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1);
		};
		return ("" + S4() + S4() + "-" + S4() + "-" + S4() + "-" + S4() + "-" + S4() + S4() + S4());
	};
	CiuisScheduler.pageOffset = function () {
	...
	CiuisScheduler.indexOf = function ($q, $r) {
		if (!$q || !$q.length) {
			return -1;
		};
	...
	CiuisScheduler.Stats = {};
	CiuisScheduler.Stats.eventObjects = 0;
	CiuisScheduler.Stats.dateObjects = 0;
	CiuisScheduler.Stats.cacheHitsCtor = 0;
	CiuisScheduler.Stats.cacheHitsParsing = 0;
	CiuisScheduler.Stats.cacheHitsTicks = 0;
	CiuisScheduler.Stats.print = function () {
		console.log("CiuisScheduler.Stats.eventObjects: " + CiuisScheduler.Stats.eventObjects);
		console.log("CiuisScheduler.Stats.dateObjects: " + CiuisScheduler.Stats.dateObjects);
		console.log("CiuisScheduler.Stats.cacheHitsCtor: " + CiuisScheduler.Stats.cacheHitsCtor);
		console.log("CiuisScheduler.Stats.cacheHitsParsing: " + CiuisScheduler.Stats.cacheHitsParsing);
		console.log("CiuisScheduler.Stats.cacheHitsTicks: " + CiuisScheduler.Stats.cacheHitsTicks);
		console.log("CiuisScheduler.Date.Cache.Ctor keys: " + Object.keys(CiuisScheduler.Date.Cache.Ctor).length);
		console.log("CiuisScheduler.Date.Cache.Parsing keys: " + Object.keys(CiuisScheduler.Date.Cache.Parsing).length);
	};
	CiuisScheduler.re = function (el, ev, $s) {
	...
	CiuisScheduler.pu = function (d) {
		...
		if (a) {
			l = a.length;
			for (i = 0; i < l; i += 1) {
				var $t = CiuisScheduler.pu(d.childNodes[i]);
			}
		}
	};
	CiuisScheduler.de = function (e) {
		if (!e) {
			return;
		};
		e.parentNode && e.parentNode.removeChild(e);
	};
	CiuisScheduler.sw = function (element) {
		if (!element) {
			return 0;
		};
		return element.offsetWidth - element.clientWidth;
	};
	CiuisScheduler.am = function () {
		if (typeof angular === "undefined") {
			return null;
		};
		if (!CiuisScheduler.am.cached) {
			CiuisScheduler.am.cached = angular.module("ciuisscheduler", []);
		};
		return CiuisScheduler.am.cached;
	};
	CiuisScheduler.Selection = function ($u, end, $v, $w) {
		this.type = 'selection';
		this.start = $u.isCiuisSchedulerDate ? $u : new CiuisScheduler.Date($u);
		this.end = end.isCiuisSchedulerDate ? end : new CiuisScheduler.Date(end);
		this.resource = $v;
		this.root = $w;
	...
	CiuisScheduler.request = function ($z, $A, $B, $C) {
		var $D = CiuisScheduler.createXmlHttp();
		if (!$D) {
			return;
		};
		$D.open("POST", $z, true);
		...
		if (typeof $B === 'object') {
			$B = CiuisScheduler.JSON.stringify($B);
		};
		$D.send($B);
	};
	CiuisScheduler.createXmlHttp = function () {
		var $E;
		try {
	...
	CiuisScheduler.Util = {};
	CiuisScheduler.Util.addClass = function ($r, name) {
		if (!$r) {
			return;
		};
		if (!$r.className) {
	...
	CiuisScheduler.Util.removeClass = function ($r, name) {
		if (!$r) {
			return;
		};
		var $F = new RegExp("(^|\\s)" + name + "($|\\s)");
	...
	CiuisScheduler.Util.isNullOrUndefined = function ($G) {
		return $G === null || typeof $G === "undefined";
	};
	CiuisScheduler.Locale = function (id, $H) {
		this.id = id;
	...
	CiuisScheduler.Locale.all = {};
	CiuisScheduler.Locale.find = function (id) {
		if (!id) {
			return null;
		};
		...
		return CiuisScheduler.Locale.all[$I];
	};
	CiuisScheduler.Locale.register = function ($J) {
		CiuisScheduler.Locale.all[$J.id] = $J;
	};
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('ca-es', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('cs-cz', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('da-dk', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('de-at', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('de-ch', {
		'dayNames': ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'],
		'dayNamesShort': ['So', 'Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa'],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('de-de', {
		'dayNames': ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'],
		'dayNamesShort': ['So', 'Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa'],
		'monthNames': ['Januar', 'Februar', 'März', 'April', 'Mai', 'Juni', 'Juli', 'August', 'September', 'Oktober', 'November', 'Dezember', ''],
		'monthNamesShort': ['Jan', 'Feb', 'Mrz', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dez', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('de-lu', {
		'dayNames': ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'],
		'dayNamesShort': ['So', 'Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa'],
		'monthNames': ['Januar', 'Februar', 'März', 'April', 'Mai', 'Juni', 'Juli', 'August', 'September', 'Oktober', 'November', 'Dezember', ''],
		'monthNamesShort': ['Jan', 'Feb', 'Mrz', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dez', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('en-au', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('en-ca', {
		'dayNames': ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
		'dayNamesShort': ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'],
		'monthNames': ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December', ''],
		'monthNamesShort': ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('en-gb', {
		'dayNames': ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
		'dayNamesShort': ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'],
		'monthNames': ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December', ''],
		'monthNamesShort': ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('en-us', {
		'dayNames': ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
		'dayNamesShort': ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'],
		'monthNames': ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December', ''],
		'monthNamesShort': ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('es-es', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('es-mx', {
		'dayNames': ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('eu-es', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('fi-fi', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('fr-be', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('fr-ch', {
		'dayNames': ['dimanche', 'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi'],
		'dayNamesShort': ['di', 'lu', 'ma', 'me', 'je', 've', 'sa'],
		'monthNames': ['janvier', 'février', 'mars', 'avril', 'mai', 'juin', 'juillet', 'août', 'septembre', 'octobre', 'novembre', 'décembre', ''],
		'monthNamesShort': ['janv.', 'févr.', 'mars', 'avr.', 'mai', 'juin', 'juil.', 'août', 'sept.', 'oct.', 'nov.', 'déc.', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('fr-fr', {
		'dayNames': ['dimanche', 'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi'],
		'dayNamesShort': ['di', 'lu', 'ma', 'me', 'je', 've', 'sa'],
		'monthNames': ['janvier', 'février', 'mars', 'avril', 'mai', 'juin', 'juillet', 'août', 'septembre', 'octobre', 'novembre', 'décembre', ''],
		'monthNamesShort': ['janv.', 'févr.', 'mars', 'avr.', 'mai', 'juin', 'juil.', 'août', 'sept.', 'oct.', 'nov.', 'déc.', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('fr-lu', {
		'dayNames': ['dimanche', 'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi'],
		'dayNamesShort': ['di', 'lu', 'ma', 'me', 'je', 've', 'sa'],
		'monthNames': ['janvier', 'février', 'mars', 'avril', 'mai', 'juin', 'juillet', 'août', 'septembre', 'octobre', 'novembre', 'décembre', ''],
		'monthNamesShort': ['janv.', 'févr.', 'mars', 'avr.', 'mai', 'juin', 'juil.', 'août', 'sept.', 'oct.', 'nov.', 'déc.', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('gl-es', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('it-it', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('it-ch', {
		'dayNames': ['domenica', 'lunedì', 'martedì', 'mercoledì', 'giovedì', 'venerdì', 'sabato'],
		'dayNamesShort': ['do', 'lu', 'ma', 'me', 'gi', 've', 'sa'],
		'monthNames': ['gennaio', 'febbraio', 'marzo', 'aprile', 'maggio', 'giugno', 'luglio', 'agosto', 'settembre', 'ottobre', 'novembre', 'dicembre', ''],
		'monthNamesShort': ['gen', 'feb', 'mar', 'apr', 'mag', 'giu', 'lug', 'ago', 'set', 'ott', 'nov', 'dic', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('ja-jp', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('nb-no', {
		'dayNames': ['søndag', 'mandag', 'tirsdag', 'onsdag', 'torsdag', 'fredag', 'lørdag'],
		'dayNamesShort': ['sø', 'ma', 'ti', 'on', 'to', 'fr', 'lø'],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('nl-nl', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('nl-be', {
		'dayNames': ['zondag', 'maandag', 'dinsdag', 'woensdag', 'donderdag', 'vrijdag', 'zaterdag'],
		'dayNamesShort': ['zo', 'ma', 'di', 'wo', 'do', 'vr', 'za'],
		'monthNames': ['januari', 'februari', 'maart', 'april', 'mei', 'juni', 'juli', 'augustus', 'september', 'oktober', 'november', 'december', ''],
		'monthNamesShort': ['jan', 'feb', 'mrt', 'apr', 'mei', 'jun', 'jul', 'aug', 'sep', 'okt', 'nov', 'dec', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('nn-no', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('pt-br', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('pl-pl', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('pt-pt', {
		'dayNames': ['domingo', 'segunda-feira', 'terça-feira', 'quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado'],
		'dayNamesShort': ['D', 'S', 'T', 'Q', 'Q', 'S', 'S'],
		'monthNames': ['janeiro', 'fevereiro', 'março', 'abril', 'maio', 'junho', 'julho', 'agosto', 'setembro', 'outubro', 'novembro', 'dezembro', ''],
		'monthNamesShort': ['jan', 'fev', 'mar', 'abr', 'mai', 'jun', 'jul', 'ago', 'set', 'out', 'nov', 'dez', ''],
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('ro-ro', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('ru-ru', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('sk-sk', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('sv-se', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('tr-tr', {
	...
	CiuisScheduler.Locale.register(new CiuisScheduler.Locale('zh-cn', {
	...
	CiuisScheduler.Locale.US = CiuisScheduler.Locale.find("en-us");
	CiuisScheduler.Date = function ($K, $L) {
		if ($K instanceof CiuisScheduler.Date) {
			return $K;
		};
		var $M;
		if (CiuisScheduler.Util.isNullOrUndefined($K)) {
			$M = CiuisScheduler.DateUtil.fromLocal().getTime();
			$K = $M;
		};
		var $N = CiuisScheduler.Date.Cache.Ctor;
		if ($N[$K]) {
			CiuisScheduler.Stats.cacheHitsCtor += 1;
			return $N[$K];
		};
		var $O = false;
		if (typeof $K === "string") {
			$M = CiuisScheduler.DateUtil.fromStringSortable($K, $L).getTime();
			$O = true;
		} else if (typeof $K === "number") {
			if (isNaN($K)) {
				throw "Cannot create CiuisScheduler.Date from NaN";
			};
			$M = $K;
		} else if ($K instanceof Date) {
			if ($L) {
				$M = CiuisScheduler.DateUtil.fromLocal($K).getTime();
			} else {
				$M = $K.getTime();
			}
		} else {
		...
		$N[$P] = this;
		$N[$M] = this;
		if ($O && $P !== $K && CiuisScheduler.DateUtil.hasTzSpec($K)) {
			$N[$K] = this;
		};
		if (Object.defineProperty && !CiuisScheduler.browser.ielt9) {
		...
		if (CiuisScheduler.Date.Config.legacyShowD) {
			this.d = new Date($M);
		};
		CiuisScheduler.Stats.dateObjects += 1;
	};
	CiuisScheduler.Date.Config = {};
	CiuisScheduler.Date.Config.legacyShowD = false;
	CiuisScheduler.Date.Cache = {};
	CiuisScheduler.Date.Cache.Parsing = {};
	CiuisScheduler.Date.Cache.Ctor = {};
	CiuisScheduler.Date.Cache.Ticks = {};
	CiuisScheduler.Date.prototype.addDays = function ($Q) {
		return new CiuisScheduler.Date(this.ticks + $Q * 24 * 60 * 60 * 1000);
	};
	CiuisScheduler.Date.prototype.addHours = function ($R) {
		return this.addTime($R * 60 * 60 * 1000);
	};
	CiuisScheduler.Date.prototype.addMilliseconds = function ($S) {
		return this.addTime($S);
	};
	CiuisScheduler.Date.prototype.addMinutes = function ($T) {
		return this.addTime($T * 60 * 1000);
	};
	CiuisScheduler.Date.prototype.addMonths = function ($U) {
		...
		d.setUTCFullYear(y);
		d.setUTCMonth(m - 1);
		var $V = new CiuisScheduler.Date(d).daysInMonth();
		d.setUTCDate(Math.min($V, $K.getUTCDate()));
		return new CiuisScheduler.Date(d);
	};
	CiuisScheduler.Date.prototype.addSeconds = function ($W) {
		return this.addTime($W * 1000);
	};
	CiuisScheduler.Date.prototype.addTime = function ($M) {
		return new CiuisScheduler.Date(this.ticks + $M);
	};
	CiuisScheduler.Date.prototype.addYears = function ($X) {
	...
	CiuisScheduler.Date.prototype.dayOfWeek = function () {
		return new Date(this.ticks).getUTCDay();
	};
	CiuisScheduler.Date.prototype.getDayOfWeek = function () {
		return new Date(this.ticks).getUTCDay();
	};
	CiuisScheduler.Date.prototype.daysInMonth = function () {
		var $K = new Date(this.ticks);
	...
	CiuisScheduler.Date.prototype.daysInYear = function () {
		var $00 = this.getYear();
		if ($00 % 4 !== 0) {
			return 365;
		};
	...
	CiuisScheduler.Date.prototype.dayOfYear = function () {
		return Math.ceil((this.getDatePart().getTime() - this.firstDayOfYear().getTime()) / 86400000) + 1;
	};
	CiuisScheduler.Date.prototype.equals = function ($01) {
		if ($01 === null) {
			return false;
		};
		if ($01 instanceof CiuisScheduler.Date) {
			return this === $01;
		} else {
			throw "The parameter must be a CiuisScheduler.Date object (CiuisScheduler.Date.equals())";
		}
	};
	CiuisScheduler.Date.prototype.firstDayOfMonth = function () {
		var d = new Date();
	...
	CiuisScheduler.Date.prototype.firstDayOfYear = function () {
		var $00 = this.getYear();
		var d = new Date();
	...
	CiuisScheduler.Date.prototype.firstDayOfWeek = function ($02) {
		var d = this;
		var $02 = $02 || 0;
		var $03 = d.dayOfWeek();
		while ($03 !== $02) {
	...
	CiuisScheduler.Date.prototype.getDay = function () {
		return new Date(this.ticks).getUTCDate();
	};
	CiuisScheduler.Date.prototype.getDatePart = function () {
		var d = new Date(this.ticks);
		d.setUTCHours(0);
		d.setUTCMinutes(0);
		d.setUTCSeconds(0);
	...
	CiuisScheduler.Date.prototype.getYear = function () {
		return new Date(this.ticks).getUTCFullYear();
	};
	CiuisScheduler.Date.prototype.getHours = function () {
		return new Date(this.ticks).getUTCHours();
	};
	CiuisScheduler.Date.prototype.getMilliseconds = function () {
		return new Date(this.ticks).getUTCMilliseconds();
	};
	CiuisScheduler.Date.prototype.getMinutes = function () {
		return new Date(this.ticks).getUTCMinutes();
	};
	CiuisScheduler.Date.prototype.getMonth = function () {
		return new Date(this.ticks).getUTCMonth();
	};
	CiuisScheduler.Date.prototype.getSeconds = function () {
		return new Date(this.ticks).getUTCSeconds();
	};
	CiuisScheduler.Date.prototype.getTotalTicks = function () {
		return this.getTime();
	};
	CiuisScheduler.Date.prototype.getTime = function () {
		return this.ticks;
	};
	CiuisScheduler.Date.prototype.getTimePart = function () {
		var $04 = this.getDatePart();
		return CiuisScheduler.DateUtil.diff(this, $04);
	};
	CiuisScheduler.Date.prototype.lastDayOfMonth = function () {
		var d = new Date(this.firstDayOfMonth().getTime());
		var length = this.daysInMonth();
		d.setUTCDate(length);
		return new CiuisScheduler.Date(d);
	};
	CiuisScheduler.Date.prototype.weekNumber = function () {
		var $05 = this.firstDayOfYear();
		var $Q = (this.getTime() - $05.getTime()) / 86400000;
		return Math.ceil(($Q + $05.dayOfWeek() + 1) / 7);
	};
	CiuisScheduler.Date.prototype.weekNumberISO = function () {
		var $06 = false;
	...
	CiuisScheduler.Date.prototype.toDateLocal = function () {
		var $K = new Date(this.ticks);
		var d = new Date();
	...
	CiuisScheduler.Date.prototype.toDate = function () {
		return new Date(this.ticks);
	};
	CiuisScheduler.Date.prototype.toJSON = function () {
		return this.value;
	};
	CiuisScheduler.Date.prototype.toString = function ($0d, $J) {
		if (typeof $0d === 'undefined') {
			return this.toStringSortable();
		};
		return new $0e($0d, $J).print(this);
	};
	CiuisScheduler.Date.prototype.toStringSortable = function () {
		return ticksToSortable(this.ticks);
	};

	function ticksToSortable($M) {
		var $N = CiuisScheduler.Date.Cache.Ticks;
		if ($N[$M]) {
			CiuisScheduler.Stats.cacheHitsTicks += 1;
			return $N[$M];
		};
		var d = new Date($M);
		var $0f;
	...
	CiuisScheduler.Date.parse = function (str, $0d, $J) {
		var p = new $0e($0d, $J);
		return p.parse(str);
	};
	CiuisScheduler.Date.today = function () {
		return new CiuisScheduler.Date().getDatePart();
	};
	CiuisScheduler.Date.fromYearMonthDay = function ($00, $Z, $03) {
		$Z = $Z || 1;
		$03 = $03 || 1;
		var d = new Date(0);
		d.setUTCFullYear($00);
	...
	CiuisScheduler.DateUtil = {};
	CiuisScheduler.DateUtil.fromStringSortable = function ($0j, $L) {
		if (!$0j) {
			throw "Can't create CiuisScheduler.Date from an empty string";
		};
		var $0k = $0j.length;
		var $K = $0k === 10;
		var $0l = $0k === 19;
		...
		if (CiuisScheduler.Date.Cache.Parsing[$0j] && !$L) {
			CiuisScheduler.Stats.cacheHitsParsing += 1;
			return CiuisScheduler.Date.Cache.Parsing[$0j];
		};
		...
		if ($K) {
			CiuisScheduler.Date.Cache.Parsing[$0j] = d;
			return d;
		};
		...
		if ($0m === ".") {
			...
			$0n = CiuisScheduler.DateUtil.getTzOffsetMinutes($0j.substring(23));
		} else {
			$0n = CiuisScheduler.DateUtil.getTzOffsetMinutes($0j.substring(19));
		};
		var dd = new CiuisScheduler.Date(d);
		if (!$L) {
			dd = dd.addMinutes(-$0n);
		};
		d = dd.toDate();
	...
	CiuisScheduler.DateUtil.getTzOffsetMinutes = function ($0j) {
		if (CiuisScheduler.Util.isNullOrUndefined($0j) || $0j === "") {
			return 0;
		};
		if ($0j === "Z") {
			return 0;
	...
	CiuisScheduler.DateUtil.hasTzSpec = function ($0j) {
		if ($0j.indexOf("+")) {
			return true;
		};
		if ($0j.indexOf("-")) {
	...
	CiuisScheduler.DateUtil.daysDiff = function ($05, $0g) {
		($05 && $0g) || (function () {
			throw "two parameters required";
		})();
		$05 = new CiuisScheduler.Date($05);
		$0g = new CiuisScheduler.Date($0g);
	...
	CiuisScheduler.DateUtil.daysSpan = function ($05, $0g) {
		($05 && $0g) || (function () {
			throw "two parameters required";
		})();
		$05 = new CiuisScheduler.Date($05);
		...
		var $0s = CiuisScheduler.DateUtil.daysDiff($05, $0g);
		if ($0g == $0g.getDatePart()) {
			$0s--;
		};
		return $0s;
	};
	CiuisScheduler.DateUtil.diff = function ($05, $0g) {
		if (!($05 && $0g && $05.getTime && $0g.getTime)) {
			throw "Both compared objects must be Date objects (CiuisScheduler.Date.diff).";
		};
		return $05.getTime() - $0g.getTime();
	};
	CiuisScheduler.DateUtil.fromLocal = function ($0t) {
		if (!$0t) {
			$0t = new Date();
		};
		var d = new Date();
	...
	CiuisScheduler.DateUtil.hours = function ($K, $0u) {
	...
	CiuisScheduler.DateUtil.max = function ($05, $0g) {
		if ($05.getTime() > $0g.getTime()) {
			return $05;
		} else {
			return $0g;
	...
	CiuisScheduler.DateUtil.min = function ($05, $0g) {
		if ($05.getTime() < $0g.getTime()) {
			return $05;
		} else {
			return $0g;
	...
	var $0e = function ($0d, $J) {
		if (typeof $J === "string") {
			$J = CiuisScheduler.Locale.find($J);
		};
		var $J = $J || CiuisScheduler.Locale.US;
		var $0w = [{
			"seq": "yyyy",
			"expr": "[0-9]{4,4\u007d",
			"str": function (d) {
			...
			"transform": function ($0x) {
				var $p = CiuisScheduler.indexOf($J.monthNames, $0x, $0y);
				if ($p < 0) {
					return null;
				};
				return $p + 1;
			...
			"transform": function ($0x) {
				var $p = CiuisScheduler.indexOf($J.monthNamesShort, $0x, $0y);
				if ($p < 0) {
					return null;
				};
				return $p + 1;
		...
		this.parse = function ($0x) {
			...
			if (CiuisScheduler.Util.isNullOrUndefined($Z)) {
				return null;
			};
			if ($Z > 12 || $Z < 1) {
				return null;
			...
			var $0M = CiuisScheduler.Date.fromYearMonthDay($00, $Z).daysInMonth();
			if ($03 < 1 || $03 > $0M) {
				return null;
			};
	...
	CiuisScheduler.Event = function ($0N, $0O, $0P) {
		var e = this;
		this.calendar = $0O;
		this.data = $0N ? $0N : {};
		this.part = $0P ? $0P : {};
		...
		this.start = function ($G) {
			if (typeof $G === 'undefined') {
				return new CiuisScheduler.Date(e.data.start);
			} else {
				this.temp().start = new CiuisScheduler.Date($G);
			}
		};
		this.end = function ($G) {
			if (typeof $G === 'undefined') {
				return new CiuisScheduler.Date(e.data.end);
			} else {
				this.temp().end = new CiuisScheduler.Date($G);
			}
		};
		this.partStart = function () {
			return new CiuisScheduler.Date(this.part.start);
		};
		this.partEnd = function () {
			return new CiuisScheduler.Date(this.part.end);
		};
...
(function () {
	...
	if (typeof CiuisScheduler.JSON.stringify !== 'function') {
		CiuisScheduler.JSON.stringify = function ($P, $13, $14) {
			var i;
			$0W = '';
			$0X = '';
			if (typeof $14 === 'number') {
...
(function () {
	var $a = function () {};
	if (typeof CiuisScheduler.Calendar !== 'undefined') {
		return;
	};
	var CiuisSchedulerCalendar = {};
	CiuisSchedulerCalendar.selectedCells = [];
	CiuisSchedulerCalendar.topSelectedCell = null;
	CiuisSchedulerCalendar.bottomSelectedCell = null;
	CiuisSchedulerCalendar.selecting = false;
	CiuisSchedulerCalendar.column = null;
	CiuisSchedulerCalendar.firstSelected = null;
	CiuisSchedulerCalendar.firstMousePos = null;
	CiuisSchedulerCalendar.originalMouse = null;
	CiuisSchedulerCalendar.originalHeight = null;
	CiuisSchedulerCalendar.originalTop = null;
	CiuisSchedulerCalendar.resizing = null;
	CiuisSchedulerCalendar.globalHandlers = false;
	CiuisSchedulerCalendar.moving = null;
	CiuisSchedulerCalendar.register = function ($b) {
		if (!CiuisSchedulerCalendar.registered) {
			CiuisSchedulerCalendar.registered = [];
		};
		var r = CiuisSchedulerCalendar.registered;
	...
	CiuisSchedulerCalendar.unregister = function ($b) {
		var a = CiuisSchedulerCalendar.registered;
		if (!a) {
			return;
		};
		var i = CiuisScheduler.indexOf(a, $b);
		if (i === -1) {
			return;
		};
		a.splice(i, 1);
	};
	CiuisSchedulerCalendar.getCellsAbove = function ($c) {
		var $d = [];
		var c = CiuisSchedulerCalendar.getColumn($c);
		var tr = $c.parentNode;
		var $e = null;
		while (tr && $e !== CiuisSchedulerCalendar.firstSelected) {
	...
	CiuisSchedulerCalendar.getCellsBelow = function ($c) {
		var $d = [];
		var c = CiuisSchedulerCalendar.getColumn($c);
		var tr = $c.parentNode;
		var $e = null;
	...
	CiuisSchedulerCalendar.getColumn = function ($c) {
		var i = 0;
	...
	CiuisSchedulerCalendar.gUnload = function (ev) {
		if (!CiuisSchedulerCalendar.registered) {
			return;
		};
		var r = CiuisSchedulerCalendar.registered;
		for (var i = 0; i < r.length; i++) {
			var c = r[i];
			c.dispose();
			CiuisSchedulerCalendar.unregister(c);
		}
	};
	CiuisSchedulerCalendar.gMouseUp = function (e) {
		if (CiuisSchedulerCalendar.resizing) {
			if (!CiuisSchedulerCalendar.resizingShadow) {
				CiuisSchedulerCalendar.resizing.style.cursor = 'default';
				document.body.style.cursor = 'default';
				CiuisSchedulerCalendar.resizing = null;
				return;
			};
			var $f = CiuisSchedulerCalendar.resizing.event;
			var $g = CiuisSchedulerCalendar.resizingShadow.clientHeight + 4;
			var top = CiuisSchedulerCalendar.resizingShadow.offsetTop;
			var $h = CiuisSchedulerCalendar.resizing.dpBorder;
			CiuisSchedulerCalendar.deleteShadow(CiuisSchedulerCalendar.resizingShadow);
			CiuisSchedulerCalendar.resizingShadow = null;
			CiuisSchedulerCalendar.resizing.style.cursor = 'default';
			$f.calendar.nav.top.style.cursor = 'auto';
			CiuisSchedulerCalendar.resizing.onclick = null;
			CiuisSchedulerCalendar.resizing = null;
			$f.calendar.eventResizeDispatch($f, $g, top, $h);
		} else if (CiuisSchedulerCalendar.moving) {
			if (!CiuisSchedulerCalendar.movingShadow) {
				CiuisSchedulerCalendar.moving = null;
				document.body.style.cursor = 'default';
				return;
			};
			var top = CiuisSchedulerCalendar.movingShadow.offsetTop;
			CiuisSchedulerCalendar.deleteShadow(CiuisSchedulerCalendar.movingShadow);
			var $f = CiuisSchedulerCalendar.moving.event;
			var $i = CiuisSchedulerCalendar.movingShadow.column;
			CiuisSchedulerCalendar.moving = null;
			CiuisSchedulerCalendar.movingShadow = null;
			$f.calendar.nav.top.style.cursor = 'auto';
			var ev = e || window.event;
			$f.calendar.eventMoveDispatch($f, $i, top, ev);
		} else if (CiuisSchedulerCalendar.selecting && CiuisSchedulerCalendar.topSelectedCell !== null) {
			var $b = CiuisSchedulerCalendar.selecting.calendar;
			CiuisSchedulerCalendar.selecting = false;
	...
	CiuisSchedulerCalendar.deleteShadow = function ($k) {
		if (!$k) {
			return;
		};
		if (!$k.parentNode) {
	...
	CiuisSchedulerCalendar.moveShadow = function ($l) {
		var $k = CiuisSchedulerCalendar.movingShadow;
		...
		$k.style.width = (CiuisSchedulerCalendar.movingShadow.parentNode.offsetWidth + 1) + 'px';
	};
	CiuisSchedulerCalendar.Calendar = function (id) {
		var $m = false;
		if (this instanceof CiuisSchedulerCalendar.Calendar && !this.$1K) {
			$m = true;
			this.$1K = true;
		};
		if (!$m) {
			throw "CiuisScheduler.Calendar() is a constructor and must be called as 'var c = new CiuisScheduler.Calendar(id);'";
		};
		var $b = this;
		this.uniqueID = null;
		this.v = '217-lite';
		...
		this.startDate = new CiuisScheduler.Date().getDatePart();
		...
		this.clearSelection = function () {
			for (var j = 0; j < CiuisSchedulerCalendar.selectedCells.length; j++) {
				var $c = CiuisSchedulerCalendar.selectedCells[j];
				if ($c) {
					if ($c.selected) {
						CiuisScheduler.de($c.selected);
		...
		this.$1L = function ($n, $o, $p) {
			...
			$q.data = $o;
			$q.header = this.$1M();
			var $r = "JSON" + CiuisScheduler.JSON.stringify($q);
		...
		this.$1N = function ($n, $o, $p) {
			...
			var $r = "JSON" + CiuisScheduler.JSON.stringify($q);
			if (this.backendUrl) {
				CiuisScheduler.request(this.backendUrl, this.callBackResponse, $r, this.ajaxError);
		...
		this.dispose = function () {
			...
			CiuisScheduler.pu(c.nav.loading);
			c.disposeMain();
			c.disposeHeader();
			c.nav.select = null;
		...
		this.$1O.locale = function () {
			var $y = CiuisScheduler.Locale.find($b.locale);
			if (!$y) {
				return CiuisScheduler.Locale.US;
			};
			return $y;
		};
		...
		this.updateView = function ($s, $t) {
			...
			if ($s.UpdateType === "Full") {
				$b.columns = $s.Columns;
				$b.days = $s.Days;
				$b.startDate = new CiuisScheduler.Date($s.StartDate);
		...
		this.eventMoveDispatch = function (e, $i, $H, ev) {
			...
			var end = e.end();
			var $K = new Date();
			if ($J instanceof CiuisScheduler.Date) {
				$J = $J.toDate();
			};
			...
			var $S = new Date();
			$S.setTime($R + $O + $P);
			var $D = new CiuisScheduler.Date($S);
		...
		this.timeRangeSelectedDispatch = function ($J, end) {
			$J = new CiuisScheduler.Date($J);
			end = new CiuisScheduler.Date(end);
			if (this.$1Q()) {
				var $C = {};
				$C.start = $J;
				$C.end = end;
		...
		this.onCellMousedown = function (ev) {
			if (CiuisSchedulerCalendar.selecting) {
				return;
			};
			...
			CiuisSchedulerCalendar.firstMousePos = CiuisScheduler.mc(ev || window.event);
			CiuisSchedulerCalendar.selecting = {};
			CiuisSchedulerCalendar.selecting.calendar = $b;
			if (CiuisSchedulerCalendar.selectedCells) {
				$b.clearSelection();
				CiuisSchedulerCalendar.selectedCells = [];
			};
			CiuisSchedulerCalendar.column = CiuisSchedulerCalendar.getColumn(this);
			CiuisSchedulerCalendar.selectedCells.push(this);
			CiuisSchedulerCalendar.firstSelected = this;
			CiuisSchedulerCalendar.topSelectedCell = this;
			CiuisSchedulerCalendar.bottomSelectedCell = this;
			$b.activateSelection();
			return false;
		};
		...
		this.mousemove = function (ev) {
			if (typeof (CiuisSchedulerCalendar) === 'undefined') {
				return;
			};
			if (!CiuisSchedulerCalendar.selecting) {
				return;
			};
			var $Y = CiuisScheduler.mc(ev || window.event);
			var $Z = CiuisSchedulerCalendar.getColumn(this);
			if ($Z !== CiuisSchedulerCalendar.column) {
				return;
			};
			$b.clearSelection();
			if ($Y.y < CiuisSchedulerCalendar.firstMousePos.y) {
				CiuisSchedulerCalendar.selectedCells = CiuisSchedulerCalendar.getCellsBelow(this);
				CiuisSchedulerCalendar.topSelectedCell = CiuisSchedulerCalendar.selectedCells[0];
				CiuisSchedulerCalendar.bottomSelectedCell = CiuisSchedulerCalendar.firstSelected;
			} else {
				CiuisSchedulerCalendar.selectedCells = CiuisSchedulerCalendar.getCellsAbove(this);
				CiuisSchedulerCalendar.topSelectedCell = CiuisSchedulerCalendar.firstSelected;
				CiuisSchedulerCalendar.bottomSelectedCell = CiuisSchedulerCalendar.selectedCells[0];
			};
			$b.activateSelection();
		};
		this.getSelection = function () {
			var $J = CiuisSchedulerCalendar.topSelectedCell.start;
			var end = CiuisSchedulerCalendar.bottomSelectedCell.end;
			return new CiuisScheduler.Selection($J, end, null, $b);
		};
		...
		this.activateColumn = function ($l) {
			$l.Start = new CiuisScheduler.Date($l.Start);
		...
		this.$1U = function (e) {
			...
			if ($o.cssClass) {
				CiuisScheduler.Util.addClass($X, $o.cssClass);
			};
			...
			$X.onmousemove = function (ev) {
				...
				var $0i = CiuisScheduler.mo3($X, ev);
				if (!$0i) {
					return;
				};
				if (CiuisSchedulerCalendar.resizing || CiuisSchedulerCalendar.moving) {
					return;
				};
				var $0j = this.isLast;
				...
				} else if (!CiuisSchedulerCalendar.resizing && !CiuisSchedulerCalendar.moving) {
			...
			$X.onmousedown = function (ev) {
				...
				if ((this.style.cursor === 'n-resize' || this.style.cursor === 's-resize') && $V === 1) {
					CiuisSchedulerCalendar.resizing = this;
					CiuisSchedulerCalendar.originalMouse = CiuisScheduler.mc(ev);
					CiuisSchedulerCalendar.originalHeight = this.offsetHeight;
					CiuisSchedulerCalendar.originalTop = this.offsetTop;
					$b.nav.top.style.cursor = this.style.cursor;
				} else if ($V === 1 && $b.eventMoveHandling !== 'Disabled') {
					CiuisSchedulerCalendar.moving = this;
					CiuisSchedulerCalendar.moving.event = this.event;
					var $0k = CiuisSchedulerCalendar.moving.helper = {};
					$0k.oldColumn = $b.columns[this.data.part.dayIndex].Value;
					CiuisSchedulerCalendar.originalMouse = CiuisScheduler.mc(ev);
					CiuisSchedulerCalendar.originalTop = this.offsetTop;
					var $0i = CiuisScheduler.mo3(this, ev);
					if ($0i) {
						CiuisSchedulerCalendar.moveOffsetY = $0i.y;
					} else {
						CiuisSchedulerCalendar.moveOffsetY = 0;
					};
			...
			if ($o.backColor) {
				...
				if (CiuisScheduler.browser.ie9 || CiuisScheduler.browser.ielt9) {
					$0l.style.filter = '';
				}
			};
			if ($o.fontColor) {
		...
		this.drawTop = function () {
			...
			if (this.cssOnly) {
				CiuisScheduler.Util.addClass(this.nav.top, this.$1T("_main"));
			} else {
		...
		this.drawHourTable = function () {
			if (!this.fasterDispose) CiuisScheduler.pu(this.nav.hourTable);
		...
		this.getScrollableHeight = function () {
			switch (this.heightSpec) {
				...
				case "BusinessHoursNoScroll":
					...
					return $0H * this.cellHeight * 2;
				default:
					throw "CiuisScheduler.Calendar: Unexpected 'heightSpec' value.";
			}
		};
		this.$1W = function () {
		...
		this.disposeMain = function () {
			...
			if (!this.fasterDispose) CiuisScheduler.pu($0x);
		};
		this.drawMain = function () {
				...
				for (var j = 0; j < cl; j++) {
					...
					if ($0y) {
						...
						if (!this.cssOnly) {
							...
							if ($0R && this.cssOnly) {
								CiuisScheduler.Util.addClass($X, $b.$1T("_cell_business"));
							};
							var $0l = document.createElement("div");
							$0l.setAttribute("unselectable", "on");
			...
			$b.nav.scrollable.onmousemove = function (ev) {
				...
				$b.coords = CiuisScheduler.mo3($0S, ev);
				var $Y = CiuisScheduler.mc(ev);
				if (CiuisSchedulerCalendar.resizing) {
					if (!CiuisSchedulerCalendar.resizingShadow) {
						CiuisSchedulerCalendar.resizingShadow = $b.createShadow(CiuisSchedulerCalendar.resizing, false, $b.shadow);
					};
					var $0T = $b.cellHeight;
					var $I = 1;
					var $0U = ($Y.y - CiuisSchedulerCalendar.originalMouse.y);
					if (CiuisSchedulerCalendar.resizing.dpBorder === 'bottom') {
						var $0V = Math.floor(((CiuisSchedulerCalendar.originalHeight + CiuisSchedulerCalendar.originalTop + $0U) + $0T / 2) / $0T) * $0T - CiuisSchedulerCalendar.originalTop + $I;
						if ($0V < $0T) $0V = $0T;
						var $04 = $b.nav.main.clientHeight;
						if (CiuisSchedulerCalendar.originalTop + $0V > $04) $0V = $04 - CiuisSchedulerCalendar.originalTop;
						CiuisSchedulerCalendar.resizingShadow.style.height = ($0V - 4) + 'px';
					} else if (CiuisSchedulerCalendar.resizing.dpBorder === 'top') {
						var $0W = Math.floor(((CiuisSchedulerCalendar.originalTop + $0U - $I) + $0T / 2) / $0T) * $0T + $I;
						if ($0W < $I) {
							$0W = $I;
						};
						if ($0W > CiuisSchedulerCalendar.originalTop + CiuisSchedulerCalendar.originalHeight - $0T) {
							$0W = CiuisSchedulerCalendar.originalTop + CiuisSchedulerCalendar.originalHeight - $0T;
						};
						var $0V = CiuisSchedulerCalendar.originalHeight - ($0W - CiuisSchedulerCalendar.originalTop) - 4;
						if ($0V < $0T) {
							$0V = $0T;
						} else {
							CiuisSchedulerCalendar.resizingShadow.style.top = $0W + 'px';
						};
						CiuisSchedulerCalendar.resizingShadow.style.height = ($0V) + 'px';
					}
				} else if (CiuisSchedulerCalendar.moving) {
					if (!CiuisSchedulerCalendar.movingShadow) {
						CiuisSchedulerCalendar.movingShadow = $b.createShadow(CiuisSchedulerCalendar.moving, !$b.ie, $b.shadow);
						CiuisSchedulerCalendar.movingShadow.style.width = (CiuisSchedulerCalendar.movingShadow.parentNode.offsetWidth + 1) + 'px';
					};
					if (!$b.coords) {
						return;
					};
					var $0T = $b.cellHeight;
					var $I = 1;
					var $0i = CiuisSchedulerCalendar.moveOffsetY;
					if (!$0i) {
						$0i = $0T / 2;
					};
					...
					var $0b = $b.nav.events;
					var $04 = $b.nav.main.clientHeight;
					var $g = parseInt(CiuisSchedulerCalendar.movingShadow.style.height);
					if ($0W + $g > $04) {
						$0W = $04 - $g;
					};
					CiuisSchedulerCalendar.movingShadow.parentNode.style.display = 'none';
					CiuisSchedulerCalendar.movingShadow.style.top = $0W + 'px';
					CiuisSchedulerCalendar.movingShadow.parentNode.style.display = '';
					var $0X = $0b.clientWidth / $0b.rows[0].cells.length;
					var $l = Math.floor(($b.coords.x - 45) / $0X);
					if ($l < 0) {
						$l = 0;
					};
					if ($l < $0b.rows[0].cells.length && $l >= 0 && CiuisSchedulerCalendar.movingShadow.column !== $l) {
						CiuisSchedulerCalendar.movingShadow.column = $l;
						CiuisSchedulerCalendar.moveShadow($0b.rows[0].cells[$l]);
					}
				}
			};
		...
		this.drawHeader = function () {
			...
			while (this.headerCreated && $0u && $0u.rows && $0u.rows.length > 0 && $0y) {
				if (!this.fasterDispose) CiuisScheduler.pu($0u.rows[0]);
				$0u.deleteRow(0);
			};
			...
			if (!$0y) {
				...
				if (!this.fasterDispose) CiuisScheduler.pu($0I.firstChild);
			};
		...
		this.$21 = function () {
			var w = CiuisScheduler.sw(this.nav.scroll);
		...
		this.registerGlobalHandlers = function () {
			if (!CiuisSchedulerCalendar.globalHandlers) {
				CiuisSchedulerCalendar.globalHandlers = true;
				CiuisScheduler.re(document, 'mouseup', CiuisSchedulerCalendar.gMouseUp);
				CiuisScheduler.re(window, 'unload', CiuisSchedulerCalendar.gUnload);
			}
		};
		this.events = {};
		...
		this.events.remove = function (e) {
			var $13 = CiuisScheduler.indexOf($b.events.list, e.data);
		...
		this.$24 = function () {
			if (this.id && this.id.tagName) {
				...
				if (!this.nav.top) {
					throw "CiuisScheduler.Calendar: The placeholder element not found: '" + id + "'.";
				}
			} else {
				throw "CiuisScheduler.Calendar() constructor requires the target element or its ID as a parameter";
			}
		};
		this.$25 = {};
		this.$25.events = [];
		...
		this.loadEvents = function () {
			...
			for (var i = 0; i < length; i++) {
				var e = $0O[i];
				e.start = new CiuisScheduler.Date(e.start);
				e.end = new CiuisScheduler.Date(e.end);
			};
			if (typeof this.onBeforeEventRender === 'function') {
				for (var i = 0; i < length; i++) {
					this.$26(i);
			...
			for (var i = 0; i < this.columns.length; i++) {
				...
				$0s.lines = [];
				$0s.blocks = [];
				var $19 = new CiuisScheduler.Date($0s.Start);
				var $1a = $19.getTime();
				...
				for (var j = 0; j < length; j++) {
					...
					var $1f = !($1e <= $1a || $1d >= $1c);
					if ($1f) {
						var ep = new CiuisScheduler.Event(e, $b);
						ep.part.dayIndex = i;
		...
		this.prepareVariables = function () {
			this.startDate = new CiuisScheduler.Date(this.startDate).getDatePart();
		};
		...
		this.$28 = function () {
			...
			CiuisSchedulerCalendar.register(this);
			this.$29();
			this.$1N('Init');
		};
		this.$20 = {};
		...
		this.$2b = function () {
			...
			for (var i = 0; i < $1u.length; i++) {
				var $1v = $1u[i];
				CiuisScheduler.Util.removeClass(this.nav.top, $1v + "_main");
			};
			this.$20.themes = [];
		};
		this.$2c = function () {
		...
		this.$22 = function () {
			if (!$b.cssOnly) {
				$b.cssOnly = true;
				window.console && window.console.log && window.console.log("CiuisScheduler: cssOnly = false mode is not supported anymore.");
			}
		};
		this.$2f = function () {
			var el = $b.nav.top;
	...
	CiuisSchedulerCalendar.Cell = function ($J, end, $l) {
		this.start = $J;
		this.end = end;
		this.column = function () {};
	};
	CiuisSchedulerCalendar.Column = function ($1y, name, $R) {
		this.value = $1y;
		this.name = name;
		this.date = new CiuisScheduler.Date($R);
	};
	CiuisScheduler.Calendar = CiuisSchedulerCalendar.Calendar;
	if (typeof jQuery !== 'undefined') {
		(function ($) {
			$.fn.ciuisschedulerCalendar = function ($1z) {
				var $1A = null;
				var j = this.each(function () {
					if (this.ciuisscheduler) {
						return;
					};
					var $1B = new CiuisScheduler.Calendar(this.id);
					this.ciuisscheduler = $1B;
					for (name in $1z) {
						$1B[name] = $1z[name];
					};
					$1B.init();
	...
	(function registerAngularModule() {
		var $1C = CiuisScheduler.am();
		if (!$1C) {
			return;
		};
		$1C.directive("ciuisschedulerCalendar", ['$parse', function ($1D) {
			return {
				"restrict": "E",
				"template": "<div></div>",
				"replace": true,
				"link": function ($1E, element, $1F) {
					var $b = new CiuisScheduler.Calendar(element[0]);
					$b.$1R.scope = $1E;
					$b.init();
					var $1G = $1F["id"];
					if ($1G) {
					...
					var $1I = $1E["$watch"];
					var $1J = $1F["config"] || $1F["ciuisschedulerConfig"];
					var $0O = $1F["events"] || $1F["ciuisschedulerEvents"];
...
(function () {
	if (typeof CiuisScheduler.DatePicker !== 'undefined') {
		return;
	};
	CiuisScheduler.DatePicker = function ($a) {
		this.v = '217-lite';
		...
		this.init = function () {
			this.date = new CiuisScheduler.Date(this.date);
			var $d = this.$n();
			if (this.resetTarget && !$d) {
				this.$o(this.date);
			};
			CiuisScheduler.re(document, "mousedown", function () {
				$c.close();
			});
		};
		this.close = function () {
		...
		this.$n = function () {
			...
			var $e = CiuisScheduler.Date.parse($d, $c.pattern);
			for (var i = 0; i < $c.patterns.length; i++) {
				if ($e) {
					return $e;
				};
				$e = CiuisScheduler.Date.parse($d, $c.patterns[i]);
			};
			return $e;
		};
		this.$o = function ($e) {
		...
		this.$m.locale = function () {
			return CiuisScheduler.Locale.find($c.locale);
		};
		this.$q = function () {
			var id = this.target;
		...
		this.show = function () {
			...
			var navigator = new CiuisScheduler.Navigator($b);
			navigator.api = 2;
			...
			var $h = CiuisScheduler.abs(element);
			...
			$k.id = $b;
			$j.appendChild($k);
			CiuisScheduler.re($j, "mousedown", function (ev) {
			...
			this.div = $j;
			var $l = $c.$n() || new CiuisScheduler.Date().getDatePart();
...
(function () {
	'use strict';
	CiuisScheduler.ModalStatic = {};
	CiuisScheduler.ModalStatic.list = [];
	CiuisScheduler.ModalStatic.hide = function () {
	...
	CiuisScheduler.ModalStatic.remove = function ($b) {
		var $c = CiuisScheduler.ModalStatic.list;
	...
	CiuisScheduler.ModalStatic.close = function ($d) {
		CiuisScheduler.ModalStatic.result($d);
		CiuisScheduler.ModalStatic.hide();
	};
	CiuisScheduler.ModalStatic.result = function (r) {
		var $c = CiuisScheduler.ModalStatic.list;
		if ($c.length > 0) {
			$c[$c.length - 1].result = r;
		}
	};
	CiuisScheduler.ModalStatic.displayed = function ($b) {
		var $c = CiuisScheduler.ModalStatic.list;
		for (var i = 0; i < $c.length; i++) {
			if ($c[i] === $b) {
				return true;
	...
	CiuisScheduler.ModalStatic.stretch = function () {
		if (this.list.length > 0) {
	...
	CiuisScheduler.Modal = function ($f) {
		...
		this.showHtml = function ($h) {
			if (CiuisScheduler.ModalStatic.displayed(this)) {
		...
		this.update = function () {
			...
			CiuisScheduler.ModalStatic.list.push(this);
		};
		...
		this.hide = function () {
			...
			CiuisScheduler.ModalStatic.remove(this);
			if (this.onClosed) {
				var $I = {};
	...
	CiuisScheduler.Modal.close = function ($d) {
		if (parent && parent.CiuisScheduler && parent.CiuisScheduler.ModalStatic) {
			parent.CiuisScheduler.ModalStatic.close($d);
		} else {
			throw "Unable to close CiuisScheduler.Modal dialog.";
		}
	};
	CiuisScheduler.Modal.opener = function () {
		return parent && parent.CiuisScheduler && parent.CiuisScheduler.ModalStatic && parent.CiuisScheduler.ModalStatic.list[parent.CiuisScheduler.ModalStatic.list.length - 1];
	};
})();
...
(function () {
	var $a = function () {};
	if (typeof CiuisScheduler.Month !== 'undefined') {
		return;
	};
	var CiuisSchedulerMonth = {};
	CiuisSchedulerMonth.Month = function ($b) {
		this.v = '217-lite';
		this.nav = {};
		var $c = this;
		this.id = $b;
		...
		this.theme = null;
		this.startDate = new CiuisScheduler.Date();
		this.width = '100%';
		this.cellHeight = 100;
		...
		this.events.remove = function (e) {
			var $j = CiuisScheduler.indexOf($c.events.list, e.data);
		...
		this.$11 = function () {
			...
			for (var x = 0; x < $o.length; x++) {
				var $f = $o[x];
				$f.start = new CiuisScheduler.Date($f.start);
				$f.end = new CiuisScheduler.Date($f.end);
				if ($f.start.getTime() > $f.end.getTime()) {
					continue;
				};
				for (var i = 0; i < this.rows.length; i++) {
					var $p = this.rows[i];
					var ep = new CiuisScheduler.Event($f, this);
					if ($p.belongsHere(ep)) {
						$p.events.push(ep);
						if (typeof this.onBeforeEventRender === 'function') {
		...
		this.drawShadow = function (x, y, $s, $v, $w, e) {
			...
			var $B = null;
			if (CiuisSchedulerMonth.resizingEvent) {
				$B = 'w-resize';
			} else if (CiuisSchedulerMonth.movingEvent) {
				$B = "move";
			};
		...
		this.$1a = function (e) {
			...
			if ($f.cssClass) {
				CiuisScheduler.Util.addClass($H, $f.cssClass);
			};
			$H.event = e;
			$H.style.width = $v + '%';
			...
			$H.onmousedown = function (ev) {
				...
				if ($I === 1) {
					CiuisSchedulerMonth.movingEvent = null;
					if (this.style.cursor === 'w-resize' || this.style.cursor === 'e-resize') {
						...
						$J.start.y = $p;
						$J.event = $H.event;
						$J.width = CiuisScheduler.DateUtil.daysSpan($J.event.start(), $J.event.end()) + 1;
						$J.direction = this.style.cursor;
						CiuisSchedulerMonth.resizingEvent = $J;
					} else if (this.style.cursor === 'move' || $c.eventMoveHandling !== 'Disabled') {
						$c.clearShadow();
						var $K = CiuisScheduler.mo2($c.nav.top, ev);
						if (!$K) {
							return;
						};
						var $L = $c.getCellBelowPoint($K.x, $K.y);
						var $y = CiuisScheduler.DateUtil.daysDiff(e.start(), $c.rows[$p].start);
						...
						$M.event = $H.event;
						$M.coords = $K;
						CiuisSchedulerMonth.movingEvent = $M;
					}
				}
			};
			$H.onmousemove = function (ev) {
				if (typeof (CiuisSchedulerMonth) === 'undefined') {
					return;
				};
				if (CiuisSchedulerMonth.movingEvent || CiuisSchedulerMonth.resizingEvent) {
					return;
				};
				var $w = CiuisScheduler.mo3($H, ev);
				if (!$w) {
					return;
				};
				var $N = 6;
		...
		this.$10 = function () {
			if (typeof this.startDate === 'string') {
				this.startDate = new CiuisScheduler.Date(this.startDate);
			};
			...
			var $S = CiuisScheduler.DateUtil.daysDiff(this.firstDate, $R) + 1;
			$Q = Math.ceil($S / 7);
			this.days = $Q * 7;
			this.rows = [];
			for (var x = 0; x < $Q; x++) {
				...
				r.getPartStart = function (ev) {
					return CiuisScheduler.DateUtil.max(this.start, ev.start());
				};
				r.getPartEnd = function (ev) {
					return CiuisScheduler.DateUtil.min(this.end, ev.end());
				};
				r.getStartColumn = function (ev) {
					var $T = this.getPartStart(ev);
					return CiuisScheduler.DateUtil.daysDiff(this.start, $T);
				};
				r.getWidth = function (ev) {
					return CiuisScheduler.DateUtil.daysSpan(this.getPartStart(ev), this.getPartEnd(ev)) + 1;
				};
		...
		this.drawTop = function () {
			...
			$02.onmousemove = function (ev) {
				if (CiuisSchedulerMonth.resizingEvent) {
					...
					var $L = $c.getCellBelowPoint($K.x, $K.y);
					$c.clearShadow();
					var $J = CiuisSchedulerMonth.resizingEvent;
					var $04 = $J.start;
					var $v, $W;
					...
					if ($K.x === CiuisSchedulerMonth.movingEvent.coords.x && $K.y === CiuisSchedulerMonth.movingEvent.coords.y) {
						return;
					};
					var $L = $c.getCellBelowPoint($K.x, $K.y);
					$c.clearShadow();
					var event = CiuisSchedulerMonth.movingEvent.event;
					var $w = CiuisSchedulerMonth.movingEvent.offset;
					var $v = $c.cellMode ? 1 : CiuisScheduler.DateUtil.daysSpan(event.start(), event.end()) + 1;
					if ($v < 1) {
						$v = 1;
					};
					$c.drawShadow($L.x, $L.y, 0, $v, $w, event);
				} else if (CiuisSchedulerMonth.timeRangeSelecting) {
					var $K = CiuisScheduler.mo2($c.nav.top, ev);
					if (!$K) {
						return;
					};
					var $L = $c.getCellBelowPoint($K.x, $K.y);
					$c.clearShadow();
					var $W = CiuisSchedulerMonth.timeRangeSelecting;
					var $06 = $W.y * 7 + $W.x;
					var $07 = $L.y * 7 + $L.x;
					...
					var $08 = $06 < $07 ? $W : $L;
					CiuisSchedulerMonth.timeRangeSelecting.from = {
						x: $08.x,
						y: $08.y
					};
					CiuisSchedulerMonth.timeRangeSelecting.width = $v;
					CiuisSchedulerMonth.timeRangeSelecting.moved = true;
		...
		this.getCellFromDate = function ($0d) {
			var $v = CiuisScheduler.DateUtil.daysDiff(this.firstDate, $0d);
			var $L = {
				x: 0,
				y: 0
			};
		...
		this.drawCell = function (x, y, $0e) {
			...
			if (this.cssOnly) {
				...
				if (!this.isWeekend(d)) {
					...
					CiuisScheduler.Util.addClass($L, $0k);
				}
			};
			...
			$L.onmousedown = function (e) {
				if ($c.timeRangeSelectedHandling !== 'Disabled') {
					$c.clearShadow();
					CiuisSchedulerMonth.timeRangeSelecting = {
						"root": $c,
						"x": x,
						"y": y,
						"from": {
			...
			$L.onclick = function () {
				var $0o = function (d) {
					var $W = new CiuisScheduler.Date(d);
					var end = $W.addDays(1);
		...
		this.$1d = function ($0q, $f, $0r) {
			...
			$0s.data = $f;
			$0s.header = this.$1e();
			var $0t = "JSON" + CiuisScheduler.JSON.stringify($0s);
			if (this.backendUrl) {
				CiuisScheduler.request(this.backendUrl, this.callBackResponse, $0t, this.ajaxError);
			}
		};
		...
		this.$1e = function () {
			...
			h.id = this.id;
			h.v = this.v;
			h.visibleStart = new CiuisScheduler.Date(this.firstDate);
		...
		this.eventClickDispatch = function (e) {
			CiuisSchedulerMonth.movingEvent = null;
			CiuisSchedulerMonth.resizingEvent = null;
			var $H = this;
			var e = e || window.event;
			var $0v = e.ctrlKey;
			e.cancelBubble = true;
		...
		this.eventMoveDispatch = function (e, x, y, $w, ev, $0y) {
			...
			var $0B = CiuisScheduler.DateUtil.diff(e.end(), $05);
			var $0C = this.getDateFromCell(x, y);
			$0C = $0C.addDays(-$w);
			var $v = CiuisScheduler.DateUtil.daysSpan(e.start(), e.end()) + 1;
			var $0D = $0C.addDays($v);
		...
		this.$1f.locale = function () {
			var $0I = CiuisScheduler.Locale.find($c.locale);
			if (!$0I) {
				return CiuisScheduler.Locale.US;
			};
			return $0I;
		...
		this.registerGlobalHandlers = function () {
			if (!CiuisSchedulerMonth.globalHandlers) {
				CiuisSchedulerMonth.globalHandlers = true;
				CiuisScheduler.re(document, 'mouseup', CiuisSchedulerMonth.gMouseUp);
			}
		};
		...
		this.$1g = function () {
			if (this.id && this.id.tagName) {
				...
				this.nav.top = document.getElementById(this.id);
				if (!this.nav.top) {
					throw "CiuisScheduler.Month: The placeholder element not found: '" + id + "'.";
				}
			} else {
				throw "CiuisScheduler.Month() constructor requires the target element or its ID as a parameter";
			}
		};
		this.$16 = function () {
			if (!$c.cssOnly) {
	...
	CiuisSchedulerMonth.gMouseUp = function (ev) {
		if (CiuisSchedulerMonth.movingEvent) {
			var $0M = CiuisSchedulerMonth.movingEvent;
			if (!$0M.event) {
				return;
			};
			...
			var $c = CiuisSchedulerMonth.movingEvent.event.calendar;
			var e = CiuisSchedulerMonth.movingEvent.event;
			...
			CiuisSchedulerMonth.movingEvent = null;
			return false;
		} else if (CiuisSchedulerMonth.resizingEvent) {
			var $0M = CiuisSchedulerMonth.resizingEvent;
			if (!$0M.event) {
				return;
			};
			if (!$0M.event.calendar) {
			...
			var $c = CiuisSchedulerMonth.resizingEvent.event.calendar;
			var e = CiuisSchedulerMonth.resizingEvent.event;
			var $W = $c.shadow.start;
			...
			if (CiuisSchedulerMonth.timeRangeSelecting.moved) {
				var $0N = CiuisSchedulerMonth.timeRangeSelecting;
				var $c = $0N.root;
				var $W = new CiuisScheduler.Date($c.getDateFromCell($0N.from.x, $0N.from.y));
				var end = $W.addDays($0N.width);
				$c.timeRangeSelectedDispatch($W, end);
				$c.clearShadow();
			};
			CiuisSchedulerMonth.timeRangeSelecting = null;
		}
	};
	CiuisScheduler.Month = CiuisSchedulerMonth.Month;
	if (typeof jQuery !== 'undefined') {
		(function ($) {
			$.fn.ciuisschedulerMonth = function ($0O) {
				var $Y = null;
				var j = this.each(function () {
					if (this.ciuisscheduler) {
						return;
					};
					var $0P = new CiuisScheduler.Month(this.id);
					this.ciuisscheduler = $0P;
					for (name in $0O) {
						$0P[name] = $0O[name];
					};
					$0P.Init();
	...
	(function registerAngularModule() {
		var $0Q = CiuisScheduler.am();
		if (!$0Q) {
			return;
		};
		$0Q.directive("ciuisschedulerMonth", ['$parse', function ($0R) {
			return {
				"restrict": "E",
				"template": "<div></div>",
				"replace": true,
				"link": function ($0S, element, $0T) {
					var $c = new CiuisScheduler.Month(element[0]);
					$c.$15.scope = $0S;
					$c.init();
					var $0U = $0T["id"];
					if ($0U) {
					...
					var $0W = $0S["$watch"];
					var $0X = $0T["config"] || $0T["ciuisschedulerConfig"];
					var $o = $0T["events"] || $0T["ciuisschedulerEvents"];
...
(function () {
	if (typeof CiuisScheduler.Navigator !== 'undefined') {
		return;
	};
	CiuisSchedulerNavigator = {};
	CiuisScheduler.Navigator = function (id) {
		this.v = '217-lite';
		var $a = this;
		this.id = id;
		this.api = 2;
		...
		this.selectionDay = new CiuisScheduler.Date().getDatePart();
		this.showMonths = 1;
		this.skipMonths = 1;
		this.command = "navigate";
		this.year = new CiuisScheduler.Date().getYear();
		this.month = new CiuisScheduler.Date().getMonth() + 1;
		this.locale = "en-us";
		...
		this.$02 = function () {
			...
			this.state = $b;
			if (!this.startDate) {
				this.startDate = new CiuisScheduler.Date(CiuisScheduler.Date.fromYearMonthDay(this.year, this.month));
			} else {
				this.startDate = new CiuisScheduler.Date(this.startDate).firstDayOfMonth();
			};
			this.calendars = [];
			this.selected = [];
			this.months = [];
		...
		this.$06 = function ($e, name) {
			...
			CiuisScheduler.Util.addClass($e, $f);
		};
		this.$07 = function ($e, name) {
			var $f = this.cssOnly ? this.$03("_" + name) : this.$03(name);
			CiuisScheduler.Util.removeClass($e, $f);
		};
		this.$08 = function (j, $g) {
			...
			var $o = $l.addMonths(1);
			var $p = CiuisScheduler.DateUtil.daysDiff($m, $o);
			var $q = 6;
			$h.rowCount = $q;
			var $r = (new CiuisScheduler.Date()).getDatePart();
			...
			if ($g.left) {
				...
				tl.innerHTML = "<span class='ion-arrow-left-c'></span>";
				tl.onclick = this.$09;
			};
			$u.appendChild(tl);
			this.tl = tl;
			...
			if ($g.right) {
				...
				tr.innerHTML = "<span class='ion-arrow-right-c'></span>";
				tr.onclick = this.$0a;
			};
			$u.appendChild(tr);
			this.tr = tr;
		...
		this.select = function ($I) {
			...
			this.selectionStart = new CiuisScheduler.Date($I).getDatePart();
		...
		this.$0k = function () {
			...
			var $p = CiuisScheduler.DateUtil.daysDiff($O, end);
			var $w = $a.selectionDay;
			if ($a.$04()) {
				var $P = {};
				$P.start = $O;
		...
		var $n = this.$0q;
		$n.locale = function () {
			return CiuisScheduler.Locale.find($a.locale);
		};
		...
		this.$0s = function () {
			if (this.id && this.id.tagName) {
				...
				this.nav.top = document.getElementById(this.id);
				if (!this.nav.top) {
					throw "CiuisScheduler.Navigator: The placeholder element not found: '" + id + "'.";
				}
			} else {
				throw "CiuisScheduler.Navigator() constructor requires the target element or its ID as a parameter";
			};
	...
	if (typeof jQuery !== 'undefined') {
		(function ($) {
			$.fn.ciuisschedulerNavigator = function ($U) {
				var $m = null;
				var j = this.each(function () {
					if (this.ciuisscheduler) {
						return;
					};
					var $V = new CiuisScheduler.Navigator(this.id);
					this.ciuisscheduler = $V;
					for (var name in $U) {
						$V[name] = $U[name];
					};
					$V.Init();
	...
	(function registerAngularModule() {
		var $W = CiuisScheduler.am();
		if (!$W) {
			return;
		};
		$W.directive("ciuisschedulerNavigator", function () {
			return {
				"restrict": "E",
				...
				"compile": function compile(element, $X) {
					...
					return function link($Y, element, $X) {
						var $a = new CiuisScheduler.Navigator(element[0]);
						$a.$0m.scope = $Y;
						$a.init();
						var $Z = $X["id"];
						if ($Z) {
						...
						var $00 = $Y["$watch"];
						$00.call($Y, $X["ciuisschedulerConfig"], function ($01) {
							for (var name in $01) {
								$a[name] = $01[name];
							};
							$a.update();
