...
(function withAngular(angular, navigator) {
  ...
  var A_DAY_IN_MILLISECONDS = 86400000
    ...
    , generateMonthAndYearHeader = function generateMonthAndYearHeader(prevButton, nextButton, preventMobile) {
      ...
      if (isMobile) {
        ...
        return [
          '<div class="_Ciuis-datepicker-calendar-header">',
            '<div class="_Ciuis-datepicker-calendar-header-middle _Ciuis-datepicker-mobile-item _Ciuis-datepicker-calendar-month">',
      ...
      return [
        '<div class="_Ciuis-datepicker-calendar-header">',
          '<div class="_Ciuis-datepicker-calendar-header-left">',
            '<a class="_Ciuis-datepicker-calendar-month-button" href="javascript:void(0)" ng-class="{\'_Ciuis-datepicker-item-hidden\': !willPrevMonthBeSelectable()}" ng-click="prevMonth()" title="{{ buttonPrevTitle }}">',
              prevButton,
            '</a>',
          '</div>',
          '<div class="_Ciuis-datepicker-calendar-header-middle _Ciuis-datepicker-calendar-month">',
            '{{month}}&nbsp;',
            '<a href="javascript:void(0)" ng-click="paginateYears(year); showYearsPagination = !showYearsPagination;">',
              '<span>',
                '{{year}}',
                '<i ng-class="{\'_Ciuis-datepicker-calendar-header-closed-pagination\': !showYearsPagination, \'_Ciuis-datepicker-calendar-header-opened-pagination\': showYearsPagination}"></i>',
              '</span>',
            '</a>',
          '</div>',
          '<div class="_Ciuis-datepicker-calendar-header-right">',
          '<a class="_Ciuis-datepicker-calendar-month-button" ng-class="{\'_Ciuis-datepicker-item-hidden\': !willNextMonthBeSelectable()}" href="javascript:void(0)" ng-click="nextMonth()" title="{{ buttonNextTitle }}">',
            nextButton,
          '</a>',
          '</div>',
        '</div>'
    ...
    , generateYearsPaginationHeader = function generateYearsPaginationHeader(prevButton, nextButton) {
      ...
      return [
        '<div class="_Ciuis-datepicker-calendar-header" ng-show="showYearsPagination">',
          '<div class="_Ciuis-datepicker-calendar-years-pagination">',
            '<a ng-class="{\'_Ciuis-datepicker-active\': y === year, \'_Ciuis-datepicker-disabled\': !isSelectableMaxYear(y) || !isSelectableMinYear(y)}" href="javascript:void(0)" ng-click="setNewYear(y)" ng-repeat="y in paginationYears track by $index">',
              '{{y}}',
            '</a>',
          '</div>',
          '<div class="_Ciuis-datepicker-calendar-years-pagination-pages">',
            '<a href="javascript:void(0)" ng-click="paginateYears(paginationYears[0])" ng-class="{\'_Ciuis-datepicker-item-hidden\': paginationYearsPrevDisabled}">',
              prevButton,
            '</a>',
            '<a href="javascript:void(0)" ng-click="paginateYears(paginationYears[paginationYears.length -1 ])" ng-class="{\'_Ciuis-datepicker-item-hidden\': paginationYearsNextDisabled}">',
              nextButton,
            '</a>',
          '</div>',
        '</div>'
    ...
    , generateDaysColumns = function generateDaysColumns() {
      ...
      return [
      '<div class="_Ciuis-datepicker-calendar-days-header">',
    ...
    , generateDays = function generateDays() {
      ...
      return [
        '<div class="_Ciuis-datepicker-calendar-body">',
          '<a href="javascript:void(0)" ng-repeat="px in prevMonthDays" class="_Ciuis-datepicker-calendar-day _Ciuis-datepicker-disabled">',
            '{{px}}',
          '</a>',
          '<a href="javascript:void(0)" ng-repeat="item in days" ng-click="setDatepickerDay(item)" ng-class="{\'_Ciuis-datepicker-active\': selectedDay === item && selectedMonth === monthNumber && selectedYear === year, \'_Ciuis-datepicker-disabled\': !isSelectableMinDate(year + \'/\' + monthNumber + \'/\' + item ) || !isSelectableMaxDate(year + \'/\' + monthNumber + \'/\' + item) || !isSelectableDate(monthNumber, year, item) || !isSelectableDay(monthNumber, year, item),\'_Ciuis-datepicker-today\': item === today.getDate() && monthNumber === (today.getMonth() + 1) && year === today.getFullYear() && !selectedDay}" class="_Ciuis-datepicker-calendar-day">',
            '{{item}}',
          '</a>',
          '<a href="javascript:void(0)" ng-repeat="nx in nextMonthDays" class="_Ciuis-datepicker-calendar-day _Ciuis-datepicker-disabled">',
            '{{nx}}',
          '</a>',
        '</div>'
      ];
    ...
    , generateHtmlTemplate = function generateHtmlTemplate(prevButton, nextButton, preventMobile) {
      ...
      var toReturn = [
        '<div class="_Ciuis-datepicker-calendar {{datepickerClass}} {{datepickerID}}" ng-class="{\'_Ciuis-datepicker-forced-to-open\': checkVisibility()}" ng-blur="hideCalendar()">',
        '</div>'
      ]
    ...
    , datepickerDirective = function datepickerDirective($window, $compile, $locale, $filter, $interpolate, $timeout) {
      ...
      var linkingFunction = function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          , theCalendar
          , defaultPrevButton = '<b class="_Ciuis-datepicker-default-button">&lang;</b>'
          , defaultNextButton = '<b class="_Ciuis-datepicker-default-button">&rang;</b>'
          ...
          , showCalendar = function showCalendar() {
            ...
            pageDatepickers = $window.document.getElementsByClassName('_Ciuis-datepicker-calendar');

            angular.forEach(pageDatepickers, function forEachDatepickerPages(value, key) {
              if (pageDatepickers[key].classList) {

                pageDatepickers[key].classList.remove('_Ciuis-datepicker-open');
              } else {

                classHelper.remove(pageDatepickers[key], '_Ciuis-datepicker-open');
              }
            });

            if (theCalendar.classList) {

              theCalendar.classList.add('_Ciuis-datepicker-open');
              if (dateFormat) {
              ...
              classHelper.add(theCalendar, '_Ciuis-datepicker-open');
            }
        ...
        $scope.hideCalendar = function hideCalendar() {
          if (theCalendar.classList) {
            theCalendar.classList.remove('_Ciuis-datepicker-open');
          } else {

            classHelper.remove(theCalendar, '_Ciuis-datepicker-open');
          }
        };
        ...
        if ($scope.datepickerAppendTo &&
          ...
          theCalendar = element[0].querySelector('._Ciuis-datepicker-calendar');
        }
  ...
  angular.module('Ciuis.datepicker', [])
